// ==================================================
// CONFIGURAÇÃO DA TIMELINE PARA CENA 2
// ==================================================
const TIMELINE_DATA = {
  currentScene: 2,
  totalScenes: 9,
...
function initTimeline() {
  console.log('🕐 Inicializando timeline da Cena 2...');
  ...
  console.log('✅ Timeline da Cena 2 inicializada');
}
...
// ==================================================
// CENA ARQUIVADA - VÍDEO LIMPO (SEM NADA NA FRENTE)
// ==================================================
function showArchivedScene(phase) {
  ...
  // EXTRAI O NÚMERO DA CENA DO phase.id
  const sceneNumber = phase.id.replace('scene', '');
  ...
  const containerElement = document.getElementById('archivedSceneContainer');
  ...
  if (containerElement) {
    // MONTA O CAMINHO DO VÍDEO DINAMICAMENTE: ../videos_scenes/scene_X.mp4
    const videoPath = `../videos_scenes/scene_${sceneNumber}.mp4`;
    
    // VÍDEO SIMPLES - SEM OVERLAY, SEM CONTROLES CUSTOMIZADOS
    containerElement.innerHTML = `
      <div class="scene-video-container">
        <video 
          id="archivedSceneVideo"
          class="scene-video"
          controls
          autoplay
          loop
    ...
    setTimeout(() => {
      const video = document.getElementById('archivedSceneVideo');
      if (video) {
        video.play().catch(e => {
          console.log('Auto-play bloqueado ou vídeo não encontrado:', e);
        });
      }
    }, 300);
  }
